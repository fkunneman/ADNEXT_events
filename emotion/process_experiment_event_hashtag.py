...
ht_tweets_dev = [lines[i] for i in matches]
with open(parts_dev) as dev_open:
    parts = dev_open.read().split('\n')

test = []
for i, instance in parts:
...
overlap = [] 
for i, tweet in train_tweets_ids:
    if tweet & ids_dev:
        overlap.append(i)
print('overlapping tweets hashtag:', overlap)

with open(parts_hashtag) as ph_open:
    train_parts_general = ph_open.read().split('\n')
...
random_tweets_ids = [x[0] for x in random_tweets_general]
overlap = [] 
for i, tweet in random_tweets_ids:
    if tweet & ids_dev:
        overlap.append(i)
print('overlapping tweets random:', overlap)

with open(parts_random) as pr_open:
    random_parts = pr_open.read().split('\n')
...
random_sample_correct_label = []
for x in random_sample:
    tokens = x.split()
with open(train, 'w', encoding = 'utf-8') as train_out:
    train_out.write('\n'.join(train_parts_general_clean))
    for x in random_sample:
